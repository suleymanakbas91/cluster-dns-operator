...
var map_DNSOverTLSConfig = map[string]string{
	"":           "DNSOverTLSConfig groups related configuration parameters used for configuring forwarding to upstream resolvers that support DNS-over-TLS",
	"transport":  "transport allows cluster administrators to opt-in to using a DNS-over-TLS connection between cluster DNS and an upstream resolver(s). Configuring TLS as the transport at this level without configuring a CABundle will result in the system certificates being used to verify the serving certificate of the upstream resolver(s).\n\nPossible values: \"\" (empty) - This is the default and results in DNS queries being sent over cleartext. \"cleartext\" - Cluster admin specified cleartext option. This results in the same functionality as an empty value but may be useful when a cluster admin wants to be more explicit about the transport, or wants to switch from \"tls\" to \"cleartext\" explicitly. \"tls\" - This indicates that DNS queries should be sent over a TLS connection. If Transport is set to TLS, you MUST also set ServerName.",
	"serverName": "serverName is the upstream server to connect to for DNS resolution. This is required when Transport is set to \"tls\" and if Transport is empty or \"cleartext\"",
	"caBundle":   "caBundle references a ConfigMap that must contain either a single CA Certificate or a CA Bundle (in the case of multiple upstreams signed by different CAs). This allows cluster administrators to provide their own CA or CA bundle for validating the certificate of upstream resolvers.\n\n1. The configmap must contain a `cabundle.crt` key 2. The value must be a PEM encoded certificate or bundle. 3. The administrator must create this configmap in the openshift-config namespace.",
}
